...
func TestNew(t *testing.T) {
	...
	if !reflect.DeepEqual(want, got) {
		t.Errorf("expected to create quiz %v got %v", want, got)
	}
}
...
func TestRun(t *testing.T) {
	t.Run("it runs the quiz", func(t *testing.T) {
		...
		if expectedResults != results {
			t.Errorf("expected right answers of %v, got %v", expectedResults, results)
		}

		expedtedOutput := "7+3: 1+1: You got 2 questions right!\n"

		if buffer.String() != expedtedOutput {
			t.Errorf("expected full output %v, got %v", expedtedOutput, buffer)
		}
	})
}
